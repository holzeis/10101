...
impl<K: DLCStoreProvider> Storage for DlcStorageProvider<K> {
    fn get_contract(&self, contract_id: &ContractId) -> Result<Option<Contract>, Error> {
        match self
            ...
            .first()
        {
            Some(res) => Ok(Some(deserialize_contract(&res.1)?)),
            None => Ok(None),
        }
    }
    ...
    fn get_contracts(&self) -> Result<Vec<Contract>, Error> {
        ...
        let contracts = contracts
            .iter()
            .filter_map(|x| match deserialize_contract(&x.1) {
    ...
    fn get_channel(&self, channel_id: &dlc_manager::ChannelId) -> Result<Option<Channel>, Error> {
        match self
            ...
            .first()
        {
            Some(res) => Ok(Some(deserialize_channel(&res.1)?)),
            None => Ok(None),
        }
    }
    ...
    fn get_chain_monitor(&self) -> Result<Option<ChainMonitor>, Error> {
        ...
        let deserialized = match serialized {
            Some(s) => Some(
                ChainMonitor::deserialize(&mut ::std::io::Cursor::new(s.1.clone()))
                    .map_err(to_storage_error)?,
            ),
            None => None,
        };
    ...
    fn get_sub_channel(
        ...
        match self
            ...
            .first()
        {
            Some(res) => Ok(Some(deserialize_sub_channel(&res.1)?)),
            None => Ok(None),
        }
    }
    ...
    fn get_sub_channel_actions(
        ...
        debug_assert!(!buf.1.is_empty());

        let len = buf.1.len();

        let mut res = Vec::new();
        let mut cursor = Cursor::new(buf.1.clone());
